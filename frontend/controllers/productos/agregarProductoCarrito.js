...
export async function agregarProductoCarrito() {
  ...
  if (carritoButton) {
    ...
    carritoButton.removeEventListener("click", handleAgregarProducto);
    carritoButton.addEventListener("click", handleAgregarProducto);
  }
}
...
async function handleAgregarProducto(event) {
  ...
  if (this.generalStock) {
    ...
    const sizeObject = this.sizes.find((item) => item.size === selectedSize);
